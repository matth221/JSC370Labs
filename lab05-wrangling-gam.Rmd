...
mid_point <- met[, .(lon_50 = quantile(lon, probs = .5, na.rm = T),
                     lat_50 = quantile(lat, probs = .5, na.rm = T)), by = STATE]
...
leaflet() %>%
  ...
  addCircles(data = mid_station, lat = ~lat, lng = ~lon, popup = "mid station", opacity = 1, fillOpacity = 1, radius = 400, color = "blue")
```
...
met[, temp_cat := fifelse(state_temperature < 20, "low-temp", fifelse(state_temperature < 25, "mid-temp", "high-temp"))]
```
...
knitr::kable(tab, caption = "cool info I want to show!")
```
...
ggplot(station_ave, aes(x = wind.sp_50, y = temp_50)) + geom_point() + geom_smooth(method = 'lm') + geom_smooth(method = 'gam', col = 2)
...
Even though the p-value is small, using linear model is kind of bad because it looks like the assumptions are violated.

Using non-linear model is more suitable in this case.
...
https://github.com/matth221/JSC370Labs
